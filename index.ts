...
interface RaceInfo {
  ...
  category: string; // La catégorie de la course (ex: CIT)
}
...
async function scrapeRaceInfo(codex: string): Promise<RaceInfo> {
  ...
  try {
    ...
    if (results.length > 1) {
      throw new Error(
        "Plusieurs résultats trouvés pour ce codex. Veuillez fournir un codex plus précis."
      );
    }
    ...
    const category = await page.evaluate(() => {
      // Cherche d'abord dans la version desktop (hidden-sm-down)
      const categoryElement = document.querySelector(
        ".g-lg-2.g-xs-2.justify-center.hidden-sm-down"
      );
      if (categoryElement && categoryElement.textContent) {
        return categoryElement.textContent.trim();
      }

      // Si pas trouvé, cherche dans la version mobile (hidden-md-up)
      const mobileElement = document.querySelector(".g-xs-12.justify-left");
      if (mobileElement && mobileElement.textContent) {
        return mobileElement.textContent.trim();
      }

      return ""; // Retourne une chaîne vide si aucune catégorie n'est trouvée
    });
    ...
    const organizerEmail = await page.evaluate(() => {
      ...
      for (const row of rows) {
        const cells = Array.from(
          row.querySelectorAll(".g-row.container > div")
        );
        if (cells.length >= 2) {
...
function displayRaceInfo(raceInfo: RaceInfo, codex: string) {
  ...
  console.log(`Catégorie: ${raceInfo.category}`);
  console.log("\nDates:");
  ...
  if (minDate?.isSame(maxDate, "day")) {
    periodText = `${minDate?.format("DD MMM YY")} ${raceInfo.title}-${
      raceInfo.category
    }`;
  } else if (minDate?.isSame(maxDate, "month")) {
    periodText = `${minDate?.format("DD")}-${maxDate?.format("DD MMM YY")}➙ ${
      raceInfo.title
    }-${raceInfo.category}`;
  } else {
    periodText = `${minDate?.format("DD MMM")} - ${maxDate?.format(
      "DD MMM YYYY"
    )}➞ ${raceInfo.title}/${raceInfo.category}`;
  }

  // Rend le texte cliquable et copiable
  console.log(
    makeTextClickableAndCopyable(periodText.replace(/\x1b\[[0-9;]*m/g, ""))
  );
